...
function checksum(letterFreq) {
  ...
  return entries.slice(0,5).map((x) => x[0]).join("");
}
var testFreq = new Map();
testFreq.set("b", 3);
testFreq.set("a", 3);
testFreq.set("d", 4);
console.assert(checksum(testFreq) == "dab");
function getLetterFreq(s) {
  var result = new Map();
  for (let c of s) {
    if (c < 'a' || c > 'z') {
      continue;
...
var actualFreq1 = Array.from(getLetterFreq("aaaaa-bbb-z-y-x-")).toString();
var expectedFreq1 = "a,5,b,3,z,1,y,1,x,1";
console.assert(actualFreq1 == expectedFreq1, "Expected " + expectedFreq1 + ", but was " + actualFreq1);

function getRealSectorId(s) {
  var encryptedName;
  var sectorId;
  var sum;
  [encryptedName, sectorId, sum] = s.split(/(\d+)/, 3);
  var computedSum = checksum(getLetterFreq(encryptedName));
  return (computedSum == sum.substring(1,6)) ? parseInt(sectorId) : 0;
}
var testInput = [
  "aaaaa-bbb-z-y-x-123[abxyz]", // is a real room because the most common letters are a (5), b (3), and then a tie between x, y, and z, which are listed alphabetically.
  "a-b-c-d-e-f-g-h-987[abcde]", // is a real room because although the letters are all tied (1 of each), the first five are listed alphabetically.
...
console.assert(getRealSectorId(testInput[0]) ==123, "the most common letters are a (5), b (3), and then a tie between x, y, and z, which are listed alphabetically.");
console.assert(getRealSectorId(testInput[1]) == 987, "although the letters are all tied (1 of each), the first five are listed alphabetically.");
console.assert(getRealSectorId(testInput[2]) == 404, "real");
console.assert(getRealSectorId(testInput[3]) == 0, "decoy");
