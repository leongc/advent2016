...
function parseInput(lines) {
  ...
  var valueTargets = []; // stack of [value, destination]
  for (let line of lines) {
    if (line.startsWith("value ")) {
      var [_, valueString, destination] = line.match(/^value (\d+) goes to (.+)$/);
      valueTargets.push([parseInt(valueString), destination]);
    } else {
      var [_, source, lowDest, highDest] = line.match(/^(.+) gives low to (.+) and high to (.+)$/);
      destinations.set(source, { name: source, lowDest: lowDest, highDest: highDest, values: [] });
    }
  }
  while (valueTargets.length > 0) {
    var [value, destination] = valueTargets.pop();
    if (destinations.has(destination)) {
      var bot = destinations.get(destination);
      bot.values.push(value);
      if (bot.values.length > 1) {
        var otherValue = bot.values[0];
        var lowValue, highValue;
        if (value < otherValue) {
          lowValue = value;
          highValue = otherValue;
        } else {
          highValue = value;
          lowValue = otherValue
        }
        valueTargets.push([lowValue, bot.lowDest]);
        valueTargets.push([highValue, bot.highDest]);
      }
    } else {
      destinations.set(destination, { name: destination, value: value });
    }
  }
  return destinations;
}
...
function findBot(destinations, value1, value2) {
  return Array.from(destinations.values())
    .find(dest => dest.values != undefined && dest.values.length == 2 &&
                   ((dest.values[0] == value1 && dest.values[1] == value2) ||
                    (dest.values[0] == value2 && dest.values[1] == value1)));
}
console.assert(findBot(parseInput(testInput), 2, 5).name == "bot 2");
