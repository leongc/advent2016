...
console.assert(hasAbba("abba"), "abba");
console.assert(hasAbba("ioxxoj"), "ioxxoj");
console.assert(!hasAbba("aaaa"), "aaaa interior characters must be different");

function getSequences(s) {
  var nonHypernets = [];
  var hypernets = [];
  for (let sequence of s.split(/(\[[^\]]+\])/)) {
    if (sequence[0] == "[") {
      hypernets.push(sequence.slice(1, -1));
    } else {
      nonHypernets.push(sequence);
    }
  }
  return [nonHypernets, hypernets];
}

function isTLS(s) {
  var nhs;
  var hs;
  [nhs, hs] = getSequences(s);
  for (let h of hs) {
    if (hasAbba(h)) {
      return false;
    }
...
console.assert(isTLS("abba[mnop]qrst"), "abba outside square brackets");
console.assert(!isTLS("abcd[bddb]xyyx"), "bddb is within square brackets, even though xyyx is outside square brackets");
console.assert(!isTLS("aaaa[qwer]tyui"), "aaaa is invalid; the interior characters must be different");
console.assert(isTLS("ioxxoj[asdfgh]zxcvbn"), "oxxo is outside square brackets, even though it's within a larger string");
